/* Assignment #: 6
 ...
 Description: creates the gui for hero pane that allows the user to
 create and see their heros
 */
...
public class HeroPane extends HBox {
	...
	public HeroPane(ArrayList<Hero> heroList) {
		...
		randombButton =  new Button("Random");
		...
		inputPane.add(strengthTextField, 1, 1);
		inputPane.add(charismaTextField, 1, 2);
		inputPane.add(damageTextField, 1, 3);
		inputPane.add(randombButton, 2, 3);
		// ^^^^^^ 1. c) ^^^^^^

		// DONE: 1. d) Bind buttons to their handlers (RandomButtonHandler and
		// AddNewHeroButtonHandler)
		// vvvvvv 1. d) vvvvvv (about 2 lines)
		randombButton.setOnAction(new RandomButtonHandler());
		...
		leftVBox.getChildren().addAll(inputPane, addHero, addHeroStatus);
		// ^^^^^^ 1. e) ^^^^^^
	...
	private class RandomButtonHandler implements EventHandler<ActionEvent> {
		public void handle(ActionEvent event) {
			...
			int min = 50;
			int max = 100;
			int damageNum = (int)Math.floor(Math.random()*(max-min+1)+min);// makes the damagenum a random number from 50 to 100
			if(damageTextField.getText().isEmpty()){// checks if the text if is empty
				damageTextField.setText(String.valueOf(damageNum));// if it is it will add the random number
			}else{
				addHeroStatus.setText("Damage is already generated");// if its not empty it will change the label
			}
			// ^^^^^^ 2. ^^^^^^
	...
	private class AddNewHeroButtonHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			String damageString = damageTextField.getText();
			// ^^^^^^ 3. a) ^^^^^^
			...
			try {
				...
				if(nameTextField.getText().isEmpty() || strengthTextField.getText().isEmpty() || charismaTextField.getText().isEmpty() || damageTextField.getText().isEmpty()){// checks if any of the textfeilds are empty
				...
				int match = 0;
				for(int i = 0; i < heroList.size(); i++){// loops through hero list to check if the name already exist
					if(nameString.equals(heroList.get(i).getName())){
						match++;
					}
				}
				if(match > 0){
				...
				int strengthTest = Integer.parseInt(strengthString);
				int charismaTest = Integer.parseInt(charismaString);
				int damageTest = Integer.parseInt(damageString);
				// ^^^^^^ 3. d) ^^^^^^
				...
				if(strengthTest < 0 || charismaTest < 0){// checks if strengh and charisma are positive
				...
				if(strengthTest + charismaTest > 100){// checks it the sum of strength and charisma is over 100
				...
				String type = heroTypeComboBox.getValue(); // once efverything is valid creates and new hero object and adds it to hero list
				heroList.add(new Hero(nameString, type, strengthTest, charismaTest, damageTest));
				// ^^^^^^ 3. g) ^^^^^^
	...
	private void updateTextArea() {
		...
		String result = "";
		for(int i = 0; i < heroList.size(); i++){// loops through herolist prinst all heros on text area
			result = result + heroList.get(i).toString() + "\n";
		}
